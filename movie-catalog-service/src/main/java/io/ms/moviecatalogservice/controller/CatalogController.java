...
import io.ms.moviecatalogservice.domain.Rating;
...
//
        ...
        UserRating ratings = webClientBuilder.build()
                .get()
                .uri("http://localhost:8081/ratingsdata/users/" + userid)
                .retrieve()
                .bodyToMono(UserRating.class)
                .block();
        System.out.println("ratings.getUserRatings() = " + ratings.getUserRatings());
        ...
        return ratings.getUserRatings().stream().map(r->{
            ...
            Movie movie = webClientBuilder.build()
                    .get()
                    .uri("localhost:8082/movies/" + r.getMovieId())
                    .retrieve()
                    .bodyToMono(Movie.class)
                    .block();

            return new CatalogItem(movie.getName(),  "Test", 123);
